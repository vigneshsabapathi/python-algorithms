...
Groups 4 bytes into a 32-bit integer, then encodes as 5 ASCII85 characters
(chr(d % 85 + 33) for each digit). Used in PostScript and PDF.
...
def ascii85_decode(data: bytes) -> bytes:
    """
    Decode Ascii85-encoded bytes back to the original bytes.
