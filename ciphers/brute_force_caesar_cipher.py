"""
Brute-Force Caesar Cipher — tries all 26 shift keys and prints each decryption.

Because there are only 26 possible keys, brute force is trivially fast.
...
def decrypt(message: str) -> None:
    """
    Print all 26 possible decryptions of a Caesar-encrypted uppercase message.
    ...
    for key in range(len(string.ascii_uppercase)):
        ...
        for symbol in message:
            if symbol in string.ascii_uppercase:
                ...
                num = (num - key) % len(string.ascii_uppercase)
