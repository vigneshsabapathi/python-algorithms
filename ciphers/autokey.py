"""
Autokey Cipher — a polyalphabetic cipher that extends the key with the plaintext.

The key is primed with an initial keyword, then the plaintext is appended to
form a running key as long as the message. Non-alphabetic characters are
preserved and skipped in key indexing.
...
def encrypt(plaintext: str, key: str) -> str:
    """
    Encrypt plaintext using the Autokey cipher.
    ...
    while plaintext_iterator < len(plaintext):
        if ord(plaintext[plaintext_iterator]) < 97 or ord(plaintext[plaintext_iterator]) > 122:
            ...
            ciphertext += chr(
                ((ord(plaintext[plaintext_iterator]) - 97 + ord(key[key_iterator])) - 97)
                % 26 + 97
            )
            key_iterator += 1
            plaintext_iterator += 1
...
def decrypt(ciphertext: str, key: str) -> str:
    """
    Decrypt ciphertext using the Autokey cipher.
    ...
    while ciphertext_iterator < len(ciphertext):
        if ord(ciphertext[ciphertext_iterator]) < 97 or ord(ciphertext[ciphertext_iterator]) > 122:
            plaintext += ciphertext[ciphertext_iterator]
        else:
            recovered = chr(
                (ord(ciphertext[ciphertext_iterator]) - ord(key[key_iterator])) % 26 + 97
            )
            plaintext += recovered
            key += recovered
            key_iterator += 1
