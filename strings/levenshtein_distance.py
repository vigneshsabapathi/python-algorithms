...
def levenshtein_distance(first_word: str, second_word: str) -> int:
    ...
    10
    """
    # Ensure the longer word is first so the inner loop is always the shorter word
    ...
    # previous_row[j] = edit distance between first_word[:0] and second_word[:j]
    previous_row = list(range(len(second_word) + 1))

    for i, c1 in enumerate(first_word):
        current_row = [i + 1]  # cost of deleting all of first_word[:i+1]

        for j, c2 in enumerate(second_word):
            insertions    = previous_row[j + 1] + 1        # insert c2
            deletions     = current_row[j] + 1             # delete c1
            substitutions = previous_row[j] + (c1 != c2)  # replace (0 if match)
...
def levenshtein_distance_optimized(first_word: str, second_word: str) -> int:
    ...
    for i, c1 in enumerate(first_word):
        # Pre-allocate full row instead of using append — avoids list resizing
