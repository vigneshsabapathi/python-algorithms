...
def naive_pattern_search(s: str, pattern: str) -> list:
    ...
    # Try every possible starting position in s
    for i in range(len(s) - pat_len + 1):
        match_found = True
        # Compare pattern character by character against the current window
        for j in range(pat_len):
            if s[i + j] != pattern[j]:
                match_found = False
                break  # early exit on first mismatch
        if match_found:
            position.append(i)  # record the start index of the match
