...
def compute_transform_tables(
    ...
    # costs[i][j] = min cost to convert source[:i] into destination[:j]
    costs = [
        [0 for _ in range(len_destination_seq + 1)] for _ in range(len_source_seq + 1)
    ]
    # ops[i][j] = the operation chosen at each cell for path reconstruction
    ops = [
        ["0" for _ in range(len_destination_seq + 1)] for _ in range(len_source_seq + 1)
    ]

    # Base case: convert source prefix to empty string → delete all chars
    for i in range(1, len_source_seq + 1):
        costs[i][0] = i * delete_cost
        ops[i][0] = f"D{source_seq[i - 1]}"

    # Base case: convert empty string to destination prefix → insert all chars
    for i in range(1, len_destination_seq + 1):
        costs[0][i] = i * insert_cost
        ops[0][i] = f"I{destination_seq[i - 1]}"

    # Fill DP table: choose the cheapest operation at each cell
    for i in range(1, len_source_seq + 1):
        for j in range(1, len_destination_seq + 1):
            if source_seq[i - 1] == destination_seq[j - 1]:
                # Characters match → copy (diagonal move)
                costs[i][j] = costs[i - 1][j - 1] + copy_cost
                ops[i][j] = f"C{source_seq[i - 1]}"
            else:
                # Characters differ → replace (diagonal move)
                costs[i][j] = costs[i - 1][j - 1] + replace_cost
                ops[i][j] = f"R{source_seq[i - 1]}" + str(destination_seq[j - 1])

            # Delete from source (upward move) — check if cheaper
            if costs[i - 1][j] + delete_cost < costs[i][j]:
                costs[i][j] = costs[i - 1][j] + delete_cost
                ops[i][j] = f"D{source_seq[i - 1]}"

            # Insert into destination (leftward move) — check if cheaper
...
def assemble_transformation(ops: list[list[str]], i: int, j: int) -> list[str]:
    ...
    """
    if i == 0 and j == 0:
        return []  # reached top-left corner — no more operations
    elif ops[i][j][0] in {"C", "R"}:
        # Copy or Replace → came from diagonal (i-1, j-1)
        seq = assemble_transformation(ops, i - 1, j - 1)
        seq.append(ops[i][j])
        return seq
    elif ops[i][j][0] == "D":
        # Delete → came from above (i-1, j)
        seq = assemble_transformation(ops, i - 1, j)
        seq.append(ops[i][j])
        return seq
    else:
        # Insert → came from left (i, j-1)
