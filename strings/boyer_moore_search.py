...
class BoyerMooreSearch:
    ...
    def bad_character_heuristic(self) -> list[int]:
        ...
        NOTE: The `i = mismatch_index - match_index` reassignment inside the
        for loop has NO EFFECT in Python — the loop variable is reset by the
        for-iterator on every iteration. The shift optimisation never fires,
        so this runs as O(n*m) brute force, not O(n/m) as claimed.
        ...
        for i in range(self.textLen - self.patLen + 1):
            ...
            if mismatch_index == -1:
                ...
                # BUG: assignment to a for-loop variable is silently ignored in
                # Python. The next iteration still uses i+1 from the range.
        return positions
