# Min heap data structure with decrease key functionality in O(log n) time
...
class MinHeap:
    """
    Min-heap with decrease_key support.
    ...
    def decrease_key(self, node, new_value):
        """Decrease the key of a node and restore heap property."""
        assert self.heap[self.idx_of_element[node]].val > new_value, (
            "newValue must be less than current value"
        )
        node.val = new_value
