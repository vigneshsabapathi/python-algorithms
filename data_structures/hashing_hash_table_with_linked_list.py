#!/usr/bin/env python3
"""
Hash Table with Linked List chaining for collision resolution.
Each slot in the table holds a deque (linked list) of values.
...
class HashTableWithLinkedList(HashTable):
    """
    Hash Table that uses linked list (deque) chaining for collision resolution.

    >>> ht = HashTableWithLinkedList(5)
    >>> ht.insert_data(10)
    >>> ht.insert_data(20)
    >>> ht.insert_data(30)
    >>> 10 in list(ht.values[0])
    True
    >>> 20 in list(ht.values[0])
    True
    >>> 30 in list(ht.values[0])
    True
    """
