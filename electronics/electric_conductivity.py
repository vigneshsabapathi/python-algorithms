#!/usr/bin/env python3
"""
Electric Conductivity — sigma = n * e * mu.

Reference: https://en.wikipedia.org/wiki/Electrical_resistivity_and_conductivity

Run:
    python -m doctest electronics/electric_conductivity.py -v
"""
...
def electric_conductivity(
    ...
    """
    Calculate any one of {conductivity, electron_conc, mobility}
    ...
    if (conductivity, electron_conc, mobility).count(0) != 1:
        ...
        return ("conductivity", mobility * electron_conc * ELECTRON_CHARGE)
    elif electron_conc == 0:
        return ("electron_conc", conductivity / (mobility * ELECTRON_CHARGE))
    else:
        return ("mobility", conductivity / (electron_conc * ELECTRON_CHARGE))
